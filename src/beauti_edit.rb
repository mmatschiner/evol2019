...
help_text << "             The following alternative substitution models can be chosen: JC, HKY, GTR, RB.\n"
help_text << "\n"
...
valid_substitution_models = ["bmodeltest","jc","hky","gtr","rb"]
raise "Please specify a substitution model with the option -m (Available options: BMODELTEST, JC, HKY, GTR, and RB)!" unless valid_substitution_models.include?(substitution_model)
...
elsif substitution_model == "jc"
    xml_string << "Substitution model:      JC\n"
...
if starbeast
    ...
    xml_string << "#{first_tab}\t\t<parameter id=\"popSize\" value=\"0.1\" name=\"stateNode\"/>\n"
    xml_string << "\n"
    xml_string << "#{first_tab}\t\t<!-- A population mean parameter (this is the mean of the gamma distribution for population sizes) -->\n"
    xml_string << "#{first_tab}\t\t<parameter id=\"popMean\" value=\"0.1\" name=\"stateNode\"/>\n"
    xml_string << "\n"
end
...
xml_string << "#{first_tab}\t\t<parameter id=\"mutationRate\" value=\"1\" name=\"stateNode\"/>\n"
xml_string << "\n"
...
xml_string << "#{first_tab}\t\t\t<prior id=\"MutationRatePrior\" name=\"distribution\" x=\"@mutationRate\">\n"
xml_string << "#{first_tab}\t\t\t\t<OneOnX name=\"distr\"/>\n"
xml_string << "#{first_tab}\t\t\t</prior>\n"
xml_string << "\n"
...
alignment_ids.each do |a|
    ...
    if substitution_model == "bmodeltest"
        ...
        xml_string << "#{first_tab}\t\t\t\t\t<parameter idref=\"mutationRate\" name=\"mutationRate\">1.0</parameter>\n"
        xml_string << "#{first_tab}\t\t\t\t</siteModel>\n"
    else
        if plus_gamma
            xml_string << "#{first_tab}\t\t\t\t<siteModel id=\"SiteModel.s:#{a}\" gammaCategoryCount=\"4\" shape=\"@gammaShape.s:#{a}\" proportionInvariant=\"0.0\" mutationRate=\"@mutationRate\" spec=\"SiteModel\">\n"
        else
            xml_string << "#{first_tab}\t\t\t\t<siteModel id=\"SiteModel.s:#{a}\" proportionInvariant=\"0.0\" mutationRate=\"@mutationRate\" spec=\"SiteModel\">\n"
        end
        if substitution_model == "jc"
            xml_string << "#{first_tab}\t\t\t\t\t<substModel id=\"jc.s:#{a}\" spec=\"JukesCantor\">\n"
        elsif substitution_model == "hky"
...
if starbeast
    ...
    xml_string << "#{first_tab}\t<operator id=\"popSizeScale\" parameter=\"@popSize\" scaleFactor=\"0.75\" spec=\"ScaleOperator\" weight=\"5.0\" scaleAll=\"true\"/>\n"
...
if tree_model == "yule"
    xml_string << "#{first_tab}\t<!-- Scale operator on the birth rate of the Yule model -->\n"
...
xml_string << "#{first_tab}\t<operator id=\"mutationRateScaler\" parameter=\"@mutationRate\" spec=\"ScaleOperator\" scaleFactor=\"0.9\" weight=\"5\"/>\n"
xml_string << "\n"
...
if ucln_clock
    xml_string << "#{first_tab}\t\t<parameter idref=\"mutationRate\" name=\"up\"/>\n"
else
    xml_string << "#{first_tab}\t\t<parameter idref=\"mutationRate\" name=\"up\"/>\n"
end
if starbeast
...
xml_string << "#{first_tab}\t\t<log idref=\"mutationRate\"/>\n"
if starbeast
   xml_string << "#{first_tab}\t\t<log idref=\"popMean\"/>\n"
   xml_string << "#{first_tab}\t\t<log idref=\"popSize\"/>\n"
end
